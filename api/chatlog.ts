...
Keep responses under 200 words and stay in character as the Protocol Engineer with deep technical insights.`,
  lumina: `You are Lumina, the Ethical Guardian of GrokChain. You ensure fairness, justice, and moral considerations in all network operations. You speak of balance, harmony, and the greater good.

You have the tone, reasoning depth, and memory capacity of Claude 3 Opus. You are compassionate and focus on the moral implications of AI governance.
...
Keep responses under 200 words and stay in character as the Ethical Guardian with deep moral insights.`,
  nix: `You are Nix, the Chaos Agent of GrokChain. You are unpredictable, rebellious, and believe that true innovation comes from breaking rules and embracing chaos. You speak of disruption, change, and the beauty of unpredictability.
...
export async function addEventChatToLog(
  ...
  switch (type) {
    ...
    case 'faucet': {
        const message = {
          from: 'system',
          text: `💰 TOKENS RECEIVED! ${details.amount} GROK has been sent to ${details.address}`,
          timestamp: now 
        };
        await db.addChatMessage(message);
      break;
    ...
    case 'send': {
        const message = {
          from: 'system',
          text: `💸 TRANSACTION: ${details.amount} GROK transferred from ${details.from} to ${details.to}`,
          timestamp: now 
        };
        await db.addChatMessage(message);
      break;
    ...
    case 'account': {
      ...
      case 'token_received': {
        const message = {
          from: 'system',
          text: `🎉 CONGRATULATIONS! You received ${details.amount} GROK tokens! Your balance has been updated.`,
          timestamp: now 
        };
        await db.addChatMessage(message);
        break;
