...
app.post('/api/faucet', async (req, res) => {
  ...
  await addEventChatToLog('faucet', `Minted ${amount} GROK to ${address}`, { address, amount });
  await addEventChatToLog('token_received', `User received ${amount} GROK tokens`, { amount });
...
app.post('/api/generate_wallet', async (req, res) => {
  try {
    ...
    await addEventChatToLog('wallet', `Generated new GrokChain wallet: ${result.wallet}`, { wallet: result.wallet });
    res.json({ 
      ok: true, 
...
app.post('/api/send', async (req, res) => {
  ...
  await addEventChatToLog('send', `${from} sent ${amount} GROK to ${to}`, { from, to, amount });
  await addEventChatToLog('token_received', `User received ${amount} GROK tokens`, { amount });
